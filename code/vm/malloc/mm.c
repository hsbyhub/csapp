...
#define GET_SIZE(p)  (GET(p) & ~0x7)                   //line:vm:mm:getsize     给一个信息块，返回块大小
#define GET_ALLOC(p) (GET(p) & 0x1)                    //line:vm:mm:getalloc    给一个信息块，返回是否已分配

/* Given block ptr bp, compute address of its header and footer */
#define HDRP(bp)       ((char *)(bp) - WSIZE)                      //line:vm:mm:hdrp    给一个用户块首地址，返回页头地址
#define FTRP(bp)       ((char *)(bp) + GET_SIZE(HDRP(bp)) - DSIZE) //line:vm:mm:ftrp    给一个用户块首地址，返回尾地址

/* Given block ptr bp, compute address of next and previous blocks */
#define NEXT_BLKP(bp)  ((char *)(bp) + GET_SIZE(((char *)(bp) - WSIZE))) //line:vm:mm:nextblkp  给一个用户快，返回下一个用户块的地址
...
/* $begin mmmalloc */
    ...
    extendsize = MAX(asize,CHUNKSIZE);                 //按片扩展
...
static void place(void *bp, size_t asize)
     /* $end mmplace-proto */
{
    size_t csize = GET_SIZE(HDRP(bp));      //取得当前块的大小 

    if ((csize - asize) >= (2*DSIZE)) {     //如果当前块比请求块大两个字，则分割
...
/* $begin mmfirstfit */
    ...
    for (bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp)) {     //heap_listp指向序言块的脚部，相当于指向一个可用负荷为0的已分配块
        if (!GET_ALLOC(HDRP(bp)) && (asize <= GET_SIZE(HDRP(bp)))) {        //采用首次匹配策略
            return bp;
        }
    }
    return NULL; /* No fit */
